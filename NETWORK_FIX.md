...
## ✅ Solution Applied
I've updated `frontend/src/config.js` to use `http://10.0.2.2:5000/api` which is the Android Emulator's special IP address to access your computer's localhost.
...
2. **Update the config file:**
   ...
   - Comment out line 5: `// export const API_URL = 'http://10.0.2.2:5000/api';`
   - Uncomment and update line 12 with your IP:
     ```javascript
     export const API_URL = 'http://YOUR_IP_ADDRESS:5000/api';
     ```
   - Example: `export const API_URL = 'http://192.168.1.5:5000/api';`
...
4. **Allow firewall access:**
   ...
   - Or manually add a rule in Windows Firewall for port 5000

### If you're using iOS Simulator:
1. Open `frontend/src/config.js`
2. Comment out the Android line
3. Uncomment: `export const API_URL = 'http://localhost:5000/api';`

## Verify Backend is Running
Make sure your backend server is running on port 5000:
- Check the terminal running `npm run dev` in the backend folder
- You should see: "Server running on port 5000"
- Test in browser: Open `http://localhost:5000` - you should see "School Management App API is running"
...
- Frontend: Configured to use `http://10.0.2.2:5000/api` (Android Emulator)
